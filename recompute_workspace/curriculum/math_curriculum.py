"""
Curriculum DeepMind Math progressif pour la phase 3C.

Objectif:
    Fournir une progression de modules texte -> texte
    (primaire -> college -> lycee -> superieur),
    avec des utilitaires pour construire les pools d'indices
    et gerer le mix de replay inter-niveaux.
"""
...
try:
    ...
    raise RuntimeError(
        "Le package `datasets` est requis. Installe-le avec: "
...
MATH_LEVEL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "college": "Arithmetique composee et theorie des nombres",
    "lycee": "Algebre et polynomes intermediaires",
    "superieur": "Calcul formel, composition et probabilites",
}
...
# Base char-level vocab (ASCII imprimable utile pour questions/reponses math).
_BASE_MATH_CHARS = (
    "0123456789"
...
def build_default_math_vocab(extra_chars: Sequence[str] | None = None) -> Dict[str, int]:
    """Construit un vocabulaire char-level deterministe pour 3C.0."""
    vocab: Dict[str, int] = {}
...
def _encode_text_to_indices(
    ...
    *,
    ...
    """Encode un texte en indices (padding/troncature inclus)."""
    if max_len <= 0:
        raise ValueError(f"max_len doit etre > 0, recu={max_len}")
...
def decode_indices(indices: Sequence[int], vocab: Mapping[str, int]) -> str:
    """Decode des indices vers texte (ignore PAD/SOS, stop sur EOS)."""
...
class MathLevelSpec:
    """Definition d'un niveau de curriculum DeepMind Math."""
...
class MathSeq2SeqDataset(Dataset):
    """
    Dataset seq2seq texte->indices (char-level) pour DeepMind Math.

    Retour par __getitem__:
        (question_indices, answer_indices)
    avec:
        question_indices: LongTensor [max_q_len]
        answer_indices:   LongTensor [max_a_len]

    Le texte brut reste disponible via `get_raw_item(idx)` pour debug.
    """
    ...
    def __init__(
        ...
        if not (len(questions) == len(answers) == len(level_names) == len(module_names) == len(level_ids)):
            raise ValueError("Toutes les sequences doivent avoir la meme longueur.")
        ...
        for required in MATH_SPECIAL_TOKENS:
            if required not in self.vocab:
                raise ValueError(f"Vocab invalide: token special manquant {required!r}")
        ...
        # Compatibilite avec le style `dataset.targets` existant.
        ...
        # Pre-encodage pour reduire la charge CPU pendant l'entrainement.
    ...
    def get_raw_item(self, idx: int) -> Dict[str, object]:
        """Retourne la version texte/metadonnees d'un exemple (debug)."""
        return {
...
def build_default_math_level_specs(
    ...
    """Construit les specs de niveaux avec modules cumulatifs."""
    if modules_by_level is not None:
        mapping = modules_by_level
    else:
        if curriculum_variant not in MATH_LEVEL_MODULE_VARIANTS:
            raise ValueError(
                f"curriculum_variant inconnu: {curriculum_variant!r}. "
                f"Choix valides: {sorted(MATH_LEVEL_MODULE_VARIANTS)}"
            )
...
def describe_math_level_specs(specs: Sequence[MathLevelSpec]) -> Dict[str, Dict[str, object]]:
    """Retourne une description JSON-serializable des niveaux."""
...
def list_required_math_modules(
    ...
    """
    Retourne la liste dedupliquee des modules requis par le curriculum.
    L'ordre suit le curriculum primaire -> superieur.
    """
...
def _set_hf_offline_mode(enabled: bool) -> Dict[str, str | None]:
    """Active/desactive temporairement le mode offline HF, retourne l'etat precedent."""
...
def check_math_dataset_local_availability(
    ...
    *,
    ...
    """
    Verifie quels modules sont deja accessibles localement (mode offline force).

    Un module est considere present si `train[:probe]` et `test[:probe]` sont lisibles
    sans acces reseau.
    """
    present: List[str] = []
    missing: List[str] = []
...
def ensure_math_dataset_available(
    ...
    *,
    ...
    """
    Gestion style FashionMNIST:
    - verifie le cache local
    - telecharge uniquement les modules manquants (si autorise)
    - re-verifie avant de retourner
    """
    ...
    if missing and auto_download_missing:
        for module_name in missing:
            # Charge les splits complets en cache local une fois pour toutes.
            load_dataset(
                "deepmind/math_dataset",
                module_name,
...
def check_math_subset_cache_availability(
    ...
    *,
    ...
    """
    Verifie la disponibilite d'un cache local "subset" par module.
    """
    present: List[str] = []
    missing: List[str] = []
    uniq_modules = list(dict.fromkeys(str(m) for m in modules))
...
def ensure_math_subset_cache(
    ...
    *,
    ...
    """
    Prepare un cache local compact (subset) par module:
    - verifie les fichiers locaux
    - telecharge via streaming uniquement les lignes necessaires si manquantes
    """
...
def _resolve_train_split(difficulty: str) -> str:
    """
    Resolve le split d'entrainement.

    DeepMind Math expose classiquement `train` et `test`.
    On accepte aussi `train-easy|medium|hard` pour compatibilite plan.
    """
    ...
    if d in {"train", "test"}:
        return d
    raise ValueError(
        f"Difficulty/split non supporte: {difficulty!r}. Utilise `train`, `test` ou `train-*`."
    )
...
def _to_text(value: object) -> str:
    ...
    if isinstance(value, str):
        text = value.strip()
        # Certains splits exposent des valeurs stringify sous forme "b'...'"
        # au lieu d'objets bytes; on les reconvertit ici.
...
def _iter_limited_rows_from_split(split_iterable, limit: int):
    """Itere un split iterable avec limite optionnelle."""
    if limit <= 0:
...
def build_math_datasets(
    ...
    """
    Charge les modules DeepMind Math et retourne (train_dataset, test_dataset).

    Args:
        modules_by_level: mapping niveau -> liste de modules
        max_samples_per_module: cap train par module
        difficulty: train split cible (accepte train-easy|medium|hard, mappe sur `train`)
        max_test_samples_per_module: cap test par module (defaut: train/10, min 200)
        use_subset_cache: lit les exemples depuis `subset_cache_dir/<module>/*.jsonl`
        subset_cache_dir: dossier du cache subset local
    """
    ...
    # Perf-safe:
    # - streaming=False (default): charge depuis cache local Arrow (rapide et offline-friendly)
    # - streaming=True: fallback remote stream si necessaire
...
def build_math_datasets_with_eval_splits(
    ...
    """
    Variante de `build_math_datasets` qui renvoie plusieurs splits d'evaluation.

    Retour:
        (train_dataset, eval_datasets)
        avec `eval_datasets["test"]` toujours present.
    """
...
def build_train_index_pools(
    ...
    """Pools d'entrainement par niveau bases sur les nouveaux modules."""
...
def build_train_index_pools_per_module(
    ...
    """Pools d'entrainement par module (new_modules du niveau courant)."""
    module_names = _extract_module_names(dataset_or_module_names)
    pools: Dict[str, np.ndarray] = {}
...
def build_eval_index_pools(
    ...
    """Pools d'evaluation par niveau bases sur les modules cumulatifs."""
    module_names = _extract_module_names(dataset_or_module_names)
    pools: Dict[str, np.ndarray] = {}
    for spec in specs:
...
def build_eval_index_pools_per_module(
    ...
    """Pools d'evaluation par module (new_modules du niveau courant)."""
    module_names = _extract_module_names(dataset_or_module_names)
    pools: Dict[str, np.ndarray] = {}
    for module_name in spec.new_modules:
        pools[str(module_name)] = _indices_for_modules(module_names, [module_name])
...
def normalize_mix(mix: Mapping[str, float]) -> Dict[str, float]:
    """Normalise un mix de poids en probabilites (somme = 1)."""
    clean = {k: max(float(v), 0.0) for k, v in mix.items() if float(v) > 0.0}
    total = sum(clean.values())
    if total <= 0:
        raise ValueError("Mix invalide: somme <= 0")
...
def interpolate_mix(
    ...
    """Interpole lineairement le mix de replay d'un niveau."""
...
def sample_mixed_indices(
    ...
    """Echantillonne un batch d'indices melanges selon `mix`."""
    if total_samples <= 0:
        raise ValueError("total_samples doit etre > 0")
