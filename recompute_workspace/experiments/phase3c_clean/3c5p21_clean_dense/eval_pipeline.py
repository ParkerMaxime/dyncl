...
def build_extra_eval_loaders(
    *,
    ...
    cap = int(max(0, getattr(cfg, "runtime_eval_cap_per_module", 0) or 0))
    for split_name, dataset in dict(getattr(bundle, "extra_eval_datasets", {})).items():
        ...
        module_pools = dict(getattr(bundle, "extra_eval_module_pools", {}).get(str(split_name), {}))
        if "primaire" in level_pools and len(level_pools["primaire"]) > 0:
            ...
            continue
        level_loader = runtime_build_loader(
            dataset,
            pool,
            int(cfg.batch_size),
            False,
        ...
        module_loaders: Dict[str, DataLoader] = {}
        for module_name, module_pool in module_pools.items():
            if len(module_pool) <= 0:
                continue
            pool_for_eval = module_pool[:cap] if cap > 0 else module_pool
            module_loaders[str(module_name)] = runtime_build_loader(
                dataset,
                pool_for_eval,
                int(cfg.batch_size),
                False,
...
def evaluate_extra_splits(
    *,
    ...
    for split_name, loader_context in loaders.items():
        if isinstance(loader_context, Mapping):
            level_loader = loader_context.get("level_loader")
            module_loaders = dict(loader_context.get("module_loaders", {}) or {})
        else:
            level_loader = loader_context
            module_loaders = {}
        if level_loader is None:
            continue
        split_metrics = evaluate_loader_with_answer_metrics(
            model=model,
            loader=level_loader,
            device=device,
            pad_id=pad_id,
        ...
        for module_name, module_loader in module_loaders.items():
            per_module_metrics[str(module_name)] = evaluate_loader_with_answer_metrics(
                model=model,
        ...
        if per_module_metrics:
            for module_name, metric in per_module_metrics.items():
                split_metrics[f"eval_seq_{module_name}"] = float(metric.get("accuracy", 0.0))
                split_metrics[f"eval_tok_{module_name}"] = float(metric.get("token_accuracy", 0.0))
                split_metrics[f"eval_seq_ans_{module_name}"] = float(
                    metric.get("eval_seq_ans", metric.get("accuracy", 0.0))
                )
                split_metrics[f"eval_tf_{module_name}"] = float(metric.get("teacher_forcing_accuracy", 0.0))
            split_metrics["primary_mastery_balanced_modules"] = float(
                sum(float(metric.get("eval_seq_ans", metric.get("accuracy", 0.0))) for metric in per_module_metrics.values())
                / max(1, len(per_module_metrics))
            )
            split_metrics["primary_mastery_balanced_modules_tf"] = float(
                sum(float(metric.get("teacher_forcing_accuracy", 0.0)) for metric in per_module_metrics.values())
                / max(1, len(per_module_metrics))
            )
        results[str(split_name)] = split_metrics
    return results
