#!/usr/bin/env python3
"""Dataset and sampler orchestration for clean 3C5dync1 college baseline."""
...
class _IndexedDatasetView(torch.utils.data.Dataset):
    ...
    def __init__(self, base_dataset: object, indices: Sequence[int]) -> None:
        ...
        # Retention views are capped (100k/module by standard), so materializing
        # these slices is acceptable and keeps downstream token inference simple.
...
def _finalize_hard_module_tiers(bundle: DatasetBundle, *, module_name: str, spec: DerivedCacheSpec) -> DatasetBundle:
    ...
    # build_or_load_bundle persists before this module-level tier finalization.
    # Persist the corrected pools so the training run can load them directly.
